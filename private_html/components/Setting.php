...
class Setting
{
    ...
    public static function set($key, $value)
    {
        $config = self::getAll();
        $config[$key] = $value;
        $res = file_put_contents(alias('@app').'/'.self::$_file, Json::encode($config));
    ...
    {
        return is_file(alias('@app').'/'.self::$_defaultsFile);
    }

    public static function getDefaults()
    {
        $filePath = alias('@app').'/'.self::$_defaultsFile;
    ...
    public static function createOrUpdateDefaults($newConfig)
    {
        if(self::existsDefaults()){
            ...
            @file_put_contents(alias('@app').'/'.self::$_defaultsFile, Json::encode($defs));
        }else
            @file_put_contents(alias('@app').'/'.self::$_defaultsFile, Json::encode($newConfig));
    }
}